 ...
 *          4：
 */
public class OcrCopy {
    private Logger logger = LoggerFactory.getLogger(OcrCopy.class);
...
//                            //发票数量检查
//                            //如果为空 直接添加  无需匹配
//                            if(!checkNull(map.get("数量")) || !checkNull(map.get("料号"))){
//                                lists.add(map);
//                            }else {
//                                boolean has = true;
//                                boolean is = false;
//                                boolean thirdly = true;
//                                List<Integer> li = new ArrayList();
//                                //1，料号一致  数量一致直接拼接放入
//                                for (int k = 0; k < list.size(); k++) {
//                                    if(! checkNull(list.get(k).get("XDCommodity_GdsQty1")) || !checkNull(list.get(k).get("XDCommodity_PartNumber"))){
//                                        lists.add(map);
//                                        break;
//                                    }else {
...
//                                            break;
//                                            //2 对多拼接 料号一致  数量不一致
//                                        }else if(deleteBlank(map.get("料号")).equals(deleteBlank(list.get(k).get("XDCommodity_PartNumber")))  &&  new Double(map.get("数量")+"") > new Double(list.get(k).get("XDCommodity_GdsQty1")+"")){
//                                            is = true;
//                                            thirdly = false;
//                                            li.add(k);
//                                        }else {  // 3 啥都没有  只添加发票数据
...
//                            }
    ...
    private List<Map>  getXDCommodity(Map<String, List<Map>> dataMap){//,String FPCommodity_PartNumber,String FPCommodity_GdsQty1
        /**
         * 箱单所有数据组装到集合中
         */
